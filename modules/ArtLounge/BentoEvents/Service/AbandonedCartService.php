...
class AbandonedCartService
{
    ...
    public function formatAbandonedCartData(CartInterface $quote): array
    {
        ...
        if ($this->config->includeRecoveryUrl($storeId)) {
            $recoveryUrl = $this->generateRecoveryUrl($quote);
            if ($recoveryUrl !== null) {
     ...
     */
    private function generateRecoveryUrl(CartInterface $quote): ?string
    {
        try {
            ...
            return $this->urlBuilder->getUrl('bento/cart/recover', [
                '_scope' => $quote->getStoreId(),
                'recover' => $token,
                'autopay' => 1,
                '_nosid' => true // Don't add session ID to URL
            ]);
        } catch (\Exception $e) {
